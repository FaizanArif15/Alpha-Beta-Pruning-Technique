...
class PuzzleState:
    ...
    def __init__(self,initial_state, parent = None, move = [0,0], minmax = None, data = None) -> None:
...
def minmax(current_state, alpha, beta):
    ...
    if current_state.minmax == "max":
        ...
        for move in possible_moves:
            ...
            value = minmax(new_state,alpha,beta)
            if beta <= alpha:
                break
            if max_value <= value:
                max_value = value
...
def solve_tictactoe(initial_state,alpha,beta):
...
if __name__ == "__main__":
    ...
    if first_move_option == 0:
        while True:
            ...
            computer_move = solve_tictactoe(initial_state,alpha,beta)
